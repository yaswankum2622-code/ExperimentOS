...
if os.path.exists("data/events.db"):
    ...
    fail("events.db was NOT created — check loader.py")
...
try:
    ...
    ok(
        f"Funnel — {stats['viewers']:,} viewers → "
...
try:
    ...
    ok(f"DB summary — A: {a['conversion_rate']}% | B: {b['conversion_rate']}%")
except Exception as e:
